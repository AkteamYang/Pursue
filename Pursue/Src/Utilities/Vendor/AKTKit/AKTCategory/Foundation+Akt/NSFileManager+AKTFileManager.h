 ...
 */
+ (NSString *)aktCreateDirectoryAtPath:(NSString *)path withType:(AKTFilePathCreateType)type;

/*
 * Create file from type (if the path exist we'll remove it and create new or just use the old, it depends on the type)
 */
+ (NSString *)aktCreateFileAtPath:(NSString *)path withType:(AKTFilePathCreateType)type;
@end
